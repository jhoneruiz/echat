...
export class MetaService {
  ...
  async createTemplate(
    ...
    try {
      ...
      if (result.status !== 200 && result.status !== 201) {
        throw new Error(
          json?.error?.message || 'Falha ao criar template na Meta',
        );
      }

      return json as ICreateTemplateResponse;
    } catch (error: any) {
  ...
  async updateTemplate(
    ...
    try {
      ...
      // Meta usa POST (no PUT) para editar un template por su ID
      ...
      if (result.status !== 200) {
        throw new Error(
          json?.error?.message || 'Falha ao atualizar template na Meta',
        );
      }
  ...
  async deleteTemplate(
    ...
    try {
      ...
      const result = await fetch(
        `${this.urlMeta}/${wabaId}/message_templates?name=${encodeURIComponent(templateName)}`,
      ...
      if (result.status !== 200) {
        throw new Error(
          json?.error?.message || 'Falha ao deletar template na Meta',
        );
      }

      return json as IDeleteTemplateResponse;
    } catch (error: any) {
