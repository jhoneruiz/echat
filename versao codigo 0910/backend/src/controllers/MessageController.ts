...
import SendWhatsAppReactionMessage from "../services/WbotServices/SendWhatsAppReactionMessage";
...
type MessageData = {
  ...
  fromMe: boolean;
  read: boolean;
  quotedMsg?: Message | { id?: number } | number | string;
  number?: string;
  isPrivate?: string;
  vCard?: Contact;
  type?: string;
...
const resolveQuotedMessage = async (
  quotedMsg: Message | { id?: number } | number | string | undefined,
  ticketId: number,
  companyId: number
  ...
  if (quotedMsg instanceof Message) {
    return quotedMsg;
  }

  let quotedId: number | undefined;

  if (typeof quotedMsg === "number") {
    quotedId = quotedMsg;
  } else if (typeof quotedMsg === "string") {
    const parsedId = Number(quotedMsg);
    quotedId = Number.isNaN(parsedId) ? undefined : parsedId;
  } else if (typeof quotedMsg === "object" && quotedMsg.id !== undefined) {
    const parsedId = Number(quotedMsg.id);
    quotedId = Number.isNaN(parsedId) ? undefined : parsedId;
  }
...
/**
 * Verifica si hay un mensaje entrante del contacto en las últimas 24 horas (ventana de servicio Meta).
 * Meta solo permite mensajes de texto libre dentro de esa ventana; fuera de ella se requiere plantilla aprobada.
 */
const isWithinMeta24hWindow = async (ticket: any): Promise<boolean> => {
  ...
  // También verificar otros tickets del mismo contacto en la misma conexión
  // (cubre el caso de ticket re-abierto donde el mensaje más reciente está en otro ticket)
  ...
  if (siblingTickets.length === 0) return false;

  const siblingIds = siblingTickets.map((t: any) => t.id);
  const siblingInbound = await Message.findOne({
    where: {
      fromMe: false,
      createdAt: { [Op.gte]: since },
      ticketId: { [Op.in]: siblingIds }
    },
    order: [["createdAt", "DESC"]]
  });
  return !!siblingInbound;
};
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  const { ticketId } = req.params;
  const { body, quotedMsg, vCard, isPrivate = "false", type, reactionTargetId }: MessageData = req.body;
  ...
  // Ventana de 24 horas de Meta: solo para mensajes de texto/media libre (no plantillas)
  if (ticket.channel === "whatsapp_oficial" && isPrivate !== "true") {
    const withinWindow = await isWithinMeta24hWindow(ticket);
    if (!withinWindow) {
      throw new AppError("ERR_META_24H_WINDOW", 422);
    }
  }
  ...
  try {
    if (type === "reaction") {
      if (!["whatsapp", "whatsapp_oficial"].includes(ticket.channel)) {
        throw new AppError("REACTION_NOT_SUPPORTED_FOR_CHANNEL", 400);
      }

      const rawQuotedId = (quotedMsg as any)?.id ?? quotedMsg ?? reactionTargetId;
      const numericQuotedId = rawQuotedId !== undefined ? Number(rawQuotedId) : undefined;
      const hasValidQuotedId = numericQuotedId !== undefined && !Number.isNaN(numericQuotedId);

      if (!hasValidQuotedId) {
        throw new AppError("REACTION_TARGET_NOT_FOUND", 404);
      }
      ...
      const reactionEmoji = typeof body === "string" ? body : "";

      if (ticket.channel === "whatsapp") {
        await SendWhatsAppReactionMessage({
          ticket,
          targetMessage,
    ...
    const resolvedQuotedMsg = await resolveQuotedMessage(quotedMsg, ticket.id, companyId);
    ...
    if (medias) {
      ...
      if (ticket.channel === "whatsapp" && isPrivate === "false") {
        await SendWhatsAppMessage({ body, ticket, quotedMsg: resolvedQuotedMsg, vCard });
      } else if (ticket.channel == 'whatsapp_oficial' && isPrivate === "false") {
        await SendWhatsAppOficialMessage({
          body, ticket, quotedMsg: resolvedQuotedMsg, type: !isNil(vCard) ? 'contacts' : 'text', media: null, vCard
        })
        ...
        const sendText = await sendFacebookMessage({ body, ticket, quotedMsg: resolvedQuotedMsg });
