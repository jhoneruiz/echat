...
import { ICreateConnectionWhatsAppOficial, ICreateConnectionWhatsAppOficialWhatsApp, ICreateTemplateOficial, ICreateTemplateOficialResponse, IDataCreateUserApiOficial, IDeleteTemplateOficialResponse, IPayloadAPIWhatsAppOficial, IResultTemplates, IReturnConnectionCreateAPIWhatsAppOficial, IReturnCreateCompanyAPIWhatsAppOficial, IReturnMessageMeta, ISendMessageOficial, IUpdateonnectionWhatsAppOficialWhatsApp, IUpdateTemplateOficial, IUserApiOficial } from "./IWhatsAppOficial.interfaces";
import fs from 'fs';
...
export const createTemplateOficial = async (multi100_token: string, dto: ICreateTemplateOficial): Promise<ICreateTemplateOficialResponse> => {
    try {
        const res = await axios.post(`${urlApi}/v1/templates-whatsapp/${multi100_token}`, dto, {
            headers: { 'Authorization': `Bearer ${token}` }
        });

        if (res.status == 200 || res.status == 201) return res.data as ICreateTemplateOficialResponse;

        throw new Error(res.data.message || 'Falha ao criar template na API Oficial');
    } catch (error) {
        console.log(`createTemplateOficial: ${error.message}`);
        throw new Error(error.message || 'Falha ao criar template na API Oficial');
    }
};

export const updateTemplateOficial = async (multi100_token: string, templateId: string, dto: IUpdateTemplateOficial): Promise<{ success: boolean }> => {
    try {
        const res = await axios.patch(`${urlApi}/v1/templates-whatsapp/${multi100_token}/${templateId}`, dto, {
            headers: { 'Authorization': `Bearer ${token}` }
        });

        if (res.status == 200 || res.status == 201) return res.data as { success: boolean };

        throw new Error(res.data.message || 'Falha ao atualizar template na API Oficial');
    } catch (error) {
        console.log(`updateTemplateOficial: ${error.message}`);
        throw new Error(error.message || 'Falha ao atualizar template na API Oficial');
    }
};

export const deleteTemplateOficial = async (multi100_token: string, templateName: string): Promise<IDeleteTemplateOficialResponse> => {
    try {
        const res = await axios.delete(`${urlApi}/v1/templates-whatsapp/${multi100_token}/${encodeURIComponent(templateName)}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });

        if (res.status == 200 || res.status == 201) return res.data as IDeleteTemplateOficialResponse;

        throw new Error(res.data.message || 'Falha ao eliminar template na API Oficial');
    } catch (error) {
        console.log(`deleteTemplateOficial: ${error.message}`);
        throw new Error(error.message || 'Falha ao eliminar template na API Oficial');
    }
};
