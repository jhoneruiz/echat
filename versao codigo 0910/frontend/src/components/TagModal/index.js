						...
						try {
							...
							// Garantir que os arquivos do backend tenham as propriedades corretas
                                                        parsedMediaFiles = parsedMediaFiles.map(file => ({
                                                                ...
                                                                name: file.originalname || file.name || 'Archivo sin nombre'
                                                        }));
						} catch (e) {
			...
			const allowedTypes = [
				...
				'video/mp4', 'video/avi', 'video/mov', 'video/webm'
                        ...
                        if (file.size > maxSize) {
                                toast.error(`El archivo ${file.name} es demasiado grande. Tamaño máximo: 10MB`);
				return false;
			}
			
                        if (!allowedTypes.includes(file.type)) {
                                toast.error(`El tipo de archivo ${file.type} no es compatible`);
				return false;
			}
