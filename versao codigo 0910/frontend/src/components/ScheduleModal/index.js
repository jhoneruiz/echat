...
const ScheduleModal = ({
  ...
  const downloadQuickMessageMedia = async (mediaPath, mediaName, mediaType) => {
    try {
      ...
      if (!companyId) {
        throw new Error("Empresa não encontrada para baixar a mídia");
      }

      const safeMediaName = encodeURIComponent(mediaName);
      const downloadUrl = `${backendUrl}/public/company${companyId}/quickMessage/${safeMediaName}`;
          ...
          setSchedule((prevState) => {
            return {
              ...
              reminderDate: data.reminderDate
                ? moment(data.reminderDate).local().format("YYYY-MM-DDTHH:mm")
                : "",
            };
          });
  ...
  const handleSaveSchedule = async (values) => {
    ...
    const scheduleData = {
      ...values,
      sendAt: toISOString(values.sendAt),
      reminderDate: toISOString(values.reminderDate),
      userId: user.id,
