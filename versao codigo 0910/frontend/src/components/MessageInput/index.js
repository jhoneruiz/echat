...
const MessageInput = ({
  ...
  const [meta24hTemplateOpen, setMeta24hTemplateOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const handleInsertQuickMessage = (event) => {
      ...
      if (!quickMessage) {
        console.error("❌ quickMessage no encontrado en el evento");
        isProcessing = false;
        return;
      }
  ...
  const handleQuickAnswersClick = useCallback(async (value) => {
    ...
    if (!ticketId) {
      console.error("❌ ticketId no encontrado");
      toastError("Error: ID del ticket no encontrado");
      return;
    }
  ...
  const handleUploadQuickMessageMedia = useCallback(async (blob, message, mediaType = null) => {
    ...
    if (!ticketId) {
      throw new Error("ID del ticket no encontrado");
    }
  ...
  const handleUploadMedia = async (mediasUpload) => {
    ...
    try {
      ...
      if (err?.response?.data?.error === "ERR_META_24H_WINDOW") {
        toastError(err);
        setShowModalMedias(false);
        setMeta24hTemplateOpen(true);
        setLoading(false);
        return;
      }
      toastError(err);
  ...
  if (mediasUpload.length > 0) {
    ...
    return (
      <>
        ...
        {meta24hTemplateOpen && (
          <TemplateModal
            open={meta24hTemplateOpen}
            handleClose={() => setMeta24hTemplateOpen(false)}
            ticketId={ticketId}
            whatsappId={whatsappId}
          />
        )}
