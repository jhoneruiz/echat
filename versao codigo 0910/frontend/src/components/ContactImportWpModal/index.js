        ...
        setTimeout(async () => {
          try {
            if (index >= contactsToImport?.length - 1) {
              setStatusMessage(`importación concluida con éxito`)
              //setContactsToImport([])
              setCurrentContact(initialContact)
            ...
            if (index % 5 === 0) {
              ...
              setStatusMessage(`importación en curso ${index} de ${contactsToImport?.length}. No salgas de esta pantalla hasta concluir la importación`)
              // toast.info(
              // );
            }
