...
const Schedules = () => {
  ...
  const calendarCulture = useMemo(() => {
    const language = (i18n.language || "pt").toLowerCase();
  ...
  const calendarMessages = useMemo(
    () => ({
      ...
      showMore: (total) => i18n.t("schedules.showMore", { total }),
    }),
    [i18n.language]
  );
      ...
      >
        <Calendar
          ...
          events={schedules.map((schedule) => ({
            ...
            start: moment(schedule.sendAt).toDate(),
            end: moment(schedule.sendAt).toDate(),
          }))}
          startAccessor="start"
          endAccessor="end"
          style={{ height: 500 }}
