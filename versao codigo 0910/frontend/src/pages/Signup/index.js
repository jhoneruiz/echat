...
const UserSchema = Yup.object().shape({
    ...
    companyName: Yup.string()
        ...
        .required("Required"),
    document: Yup.string()
        .test('document-format', 'El CPF debe tener 11 dígitos y el CNPJ debe tener 14 dígitos', function (value) {
            if (!value) return false;
            const numbers = value.replace(/\D/g, '');
            return numbers.length === 11 || numbers.length === 14;
        })
        .required("El CPF o CNPJ es obligatorio"),
...
const SignUp = () => {
            ...
            try {
                ...
                if (data.valid && data.data.nome) {
                    if (data.data.tipo === 'cnpj') {
                        ...
                        toast.success("¡CNPJ válido! Nombre de la empresa completado automáticamente.");
                    } else {
                        toast.success("¡CPF válido!");
                    }
                    setLastValidatedDocument(cleanDocument);
                    ...
                    setLastValidatedDocument(cleanDocument);
                    toast.error("Documento inválido o no encontrado en la Receita Federal");
                    return false;
                }
            } catch (error) {
                setValidatingCnpj(false);
                console.error("Erro ao validar documento:", error);
                toast.error("Error al validar el documento. Inténtalo de nuevo.");
                setLastValidatedDocument(cleanDocument);
                return false;
            }
        }
    ...
    return (
        <React.Fragment>
            <Helmet>
                <title>{appName ? `${appName} - Registro` : "Equipechat - Registro"}</title>
                                    ...
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item xs={12}>
                                            <Field
                                                ...
                                                helperText={touched.document && errors.document || "Ingresa CPF (11 dígitos) o CNPJ (14 dígitos)"}
