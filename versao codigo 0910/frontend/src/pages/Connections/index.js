...
const Connections = () => {
  ...
  useEffect(() => {
    ...
    socket.on(`transferTickets-${user.companyId}`, (data) => {
      ...
      if (data.action === "completed") {
        ...
        toast.success(`¡Transferencia completada! ${data.transferred} tickets transferidos con éxito.`);
        handleCloseTransferModal();
      }
      if (data.action === "error") {
        setTransferProgressModalOpen(false);
        setTransferProgress({ current: 0, total: 0, percentage: 0 });
        toast.error("Error en la transferencia de tickets.");
      }
    });
  ...
  const handleTransferTickets = async () => {
    if (!sourceConnection || !targetConnection) {
      toast.error("Selecciona las conexiones de origen y destino");
      return;
    }

    if (sourceConnection === targetConnection) {
      toast.error("Las conexiones de origen y destino deben ser diferentes");
      return;
    }
    ...
    try {
      ...
      if (response.data.requiresProgress) {
        ...
        toast.success(`¡Tickets transferidos con éxito! ${response.data.transferred || 0} tickets transferidos.`);
        handleCloseTransferModal();
      }
    } catch (err) {
      toastError(err);
  ...
  return (
    <MainContainer>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Transferencia de tickets</DialogTitle>
        <DialogContent>
          <Typography variant="body1" style={{ marginBottom: 24, lineHeight: 1.6 }}>
            Para transferir los tickets, selecciona la conexión de <strong>origen</strong> (desde donde se moverán los tickets)
            y la conexión de <strong>destino</strong> (a donde se transferirán los tickets).
            Todas las atenciones activas de la conexión de origen se moverán a la conexión de destino.
          </Typography>
     ...
     <Dialog
       ...
       >
         <DialogTitle>Transfiriendo tickets</DialogTitle>
         <DialogContent>
           <div style={{ textAlign: 'center', padding: '20px 0' }}>
             ...
             <Typography variant="body2" color="textSecondary" style={{ marginTop: 8 }}>
               Por favor, espera mientras se transfieren los tickets...
             </Typography>
           </div>
         </DialogContent>
       </Dialog>
       ...
       >
         ...
         <DialogContent>
           <Typography variant="body1" style={{ marginBottom: 16 }}>
             Antes de eliminar esta conexión, ¿ya transferiste los tickets a otra conexión?
           </Typography>
         </DialogContent>
         <DialogActions>
